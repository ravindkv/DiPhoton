...
void ggToaaMG::Loop_MG()
{
	...
	for (Long64_t nentry=0; nentry<nentries; nentry++) 
	{
        ...
        for(int i=0; i<Particle_size; i++){
          if(Particle_PID[i]==22 && Particle_Status[i]==1){
 	        E.push_back(Particle_E[i]);
 	        Px.push_back(Particle_Px[i]);
 	        Py.push_back(Particle_Py[i]);
 	        Pz.push_back(Particle_Pz[i]);
 	        Pt.push_back(Particle_PT[i]);
 	        
            Eta.push_back(Particle_Eta[i]);
 	        Phi.push_back(Particle_Phi[i]);
          }
          if(Particle_PID[i]!=22 && Particle_Status[i]==1){
              //cout<<"Particle_PID = "<<Particle_PID[i]<<endl;
              Ht = Ht+ Particle_PT[i];
          }
        }
        //cout<<"Ht = "<<Ht<<endl;  
        Float_t Mgg2;
        Float_t prod_PT = Pt[0]*Pt[1];
        Float_t diff_Eta = cosh(Eta[0] -Eta[1]);
        Float_t diff_Phi = cos(Phi[0]-Phi[1]);
        //https://en.wikipedia.org/wiki/Invariant_mass
        Mgg2 = sqrt(abs(2*(prod_PT*(diff_Eta - diff_Phi))));
        
        Mgg = sqrt(2*(E[0]*E[1] - Px[0]*Px[1] - Py[0]*Py[1] - Pz[0]*Pz[1]));
        //cout<<"both Mgg ="<<Mgg2<<"\t"<<Mgg<<endl;
        
        //Apply ATLAS and Ht cuts
        //if(Pt[0] >= 0.4*Mgg && Pt[1] >= 0.3*Mgg && Mgg >= 200){    
        if(Pt[0] >= 0.4*Mgg && Pt[1] >= 0.3*Mgg && Mgg >= 200 && Ht >= 50){    
    	
            h->Fill(Mgg); // Create a histogram of Mgg
            //cout<<"Pt1 = "<<Pt[0]<<"\t"<<"Pt2 = "<<Pt[1]<<"\t"<<"Mgg = "<<"\t"<<Mgg<<endl;
        }
    ...
    plotBinErrors->SetTitle("MG: g g -> a a [QCD], 100K");
